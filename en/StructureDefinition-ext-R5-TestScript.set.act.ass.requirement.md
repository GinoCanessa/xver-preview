# ExtensionTestScript_Setup_Action_Assert_Requirement - Extensions for Using Data Elements from FHIR R5 in FHIR R4 v0.0.1-snapshot-3
  ...
  "differential" : {
    "element" : [
      ...
      },
      {
        ...
        "requirements" : "Element `TestScript.setup.action.assert.requirement.link[x]` has a context of TestScript.setup.action.assert based on following the parent source element upwards and mapping to `TestScript`.\nNote available implied context: `TestScript.test.action.assert` because `TestScript.test.action.assert` is defined via a content reference to `TestScript.setup.action.assert`.",
        "min" : 0,
        "max" : "1",
        "base" : {
          "path" : "Extension.extension",
