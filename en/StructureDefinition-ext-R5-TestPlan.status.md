# ExtensionTestPlan_Status - Extensions for Using Data Elements from FHIR R5 in FHIR R4 v0.1.0

## Extension: R5: draft | active | retired | unknown (new) 

R5: `TestPlan.status` (new:code)
...
You can also check for [usages in the FHIR IG Statistics](https://packages2.fhir.org/xig/hl7.fhir.uv.xver-r5.r4|current/StructureDefinition/ext-R5-TestPlan.status)
...
Other representations of profile: [CSV](../StructureDefinition-ext-R5-TestPlan.status.csv), [Excel](../StructureDefinition-ext-R5-TestPlan.status.xlsx), [Schematron](../StructureDefinition-ext-R5-TestPlan.status.sch) 
...
```json
{
  ...
  "description" : "R5: `TestPlan.status` (new:code)",
  "jurisdiction" : [{
    "coding" : [{
  ...
  "purpose" : "This extension is part of the cross-version definitions generated to enable use of the\r\nelement `TestPlan.status` as defined in FHIR R5\r\nin FHIR R4.\r\n\r\nThe source element is defined as:\r\n`TestPlan.status` 1..1 `code`\r\n\r\nAcross FHIR versions, the element set has been mapped as:\r\n* R5 `TestPlan.status` 1..1 `code`\r\n\r\nFollowing are the generation technical comments:\r\nElement `TestPlan.status` is not mapped to FHIR R4, since FHIR R5 `TestPlan` is not mapped.\nElement `TestPlan.status` has no mapping targets in FHIR R4. Typically, this is because the element has been added (is a new element).",
  "fhirVersion" : "4.0.1",
  "mapping" : [{
    "identity" : "rim",
    "uri" : "http://hl7.org/v3",
